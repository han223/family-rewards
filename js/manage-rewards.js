...
(function () {
  ...
  function showListShell(){
    ...
    el.querySelector(".mr-close").onclick=closeManager; el.querySelector(".mr-add").onclick=()=>openForm(null); return el;
  }
  ...
  async function refreshRewards(){
    ...
    if(typeof window.renderFilters==="function")window.renderFilters(); if(typeof window.renderProducts==="function")window.renderProducts(); return true;
  }

  function renderList(){
    const list=document.getElementById("mrList");if(!list)return;const all=[...(window.rewards||[])].sort((a,b)=>(a.sort_order??a.id)-(b.sort_order??b.id));
  ...
  async function openManager(){if(!parentOnly())return;showListShell();const el=overlay();el.style.display="flex";await refreshRewards();renderList();}
  function closeManager(){const el=document.getElementById("manageRewardsOverlay");if(el)el.style.display="none";editingId=null;pendingFile=null;}
  function categories(){return [...new Set((window.rewards||[]).map(r=>String(r.category||"").trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));}

  function openForm(id){
    if(!parentOnly())return;editingId=id==null?null:Number(id);pendingFile=null;const r=editingId==null?null:(window.rewards||[]).find(x=>Number(x.id)===editingId);if(editingId!=null&&!r)return;
    const el=overlay(),cats=categories();el.querySelector(".mr-modal").innerHTML=`<div class="mr-head"><div><h2>${r?"Edit Reward":"Add Reward"}</h2><p>${r?"Update this reward.":"Create a new reward for the store."}</p></div><button class="mr-close" type="button">×</button></div><div class="mr-form"><div class="mr-field"><label>Reward name</label><input id="mrName" value="${esc(r?.name||"")}" placeholder="e.g. Movie Night"></div><div class="mr-field"><label>Points</label><input id="mrCost" type="number" min="0" step="1" value="${Number(r?.cost||0)}"></div><div class="mr-field"><label>Category</label><input id="mrCategory" list="mrCategoryList" value="${esc(r?.category||"")}" placeholder="e.g. Snacks"><datalist id="mrCategoryList">${cats.map(c=>`<option value="${esc(c)}">`).join("")}</datalist><span class="mr-category-note">Choose an existing category or type a new one.</span></div><div class="mr-field"><label>Stock</label><input id="mrStock" type="number" min="0" step="1" value="${r?.stock==null?"":Number(r.stock)}" placeholder="Leave empty for unlimited"></div><div class="mr-field full"><label>Photo</label><div class="mr-photo"><div class="mr-preview" id="mrPreview">${r?.image?`<img src="${esc(imageUrl(r.image)||"")}" alt="">`:esc(r?.icon||"🎁")}</div><button type="button" id="mrPhotoButton">📷 Choose photo</button><input id="mrPhotoInput" type="file" accept="image/*" hidden></div></div></div><div class="mr-form-actions"><button class="mr-back" type="button">← Back to Rewards</button><button class="mr-save" type="button">Save Reward</button></div>`;
    el.querySelector(".mr-close").onclick=closeManager;el.querySelector(".mr-back").onclick=openManager;el.querySelector("#mrPhotoButton").onclick=()=>el.querySelector("#mrPhotoInput").click();el.querySelector("#mrPhotoInput").onchange=e=>{pendingFile=e.target.files?.[0]||null;if(pendingFile){const url=URL.createObjectURL(pendingFile);el.querySelector("#mrPreview").innerHTML=`<img src="${url}" alt="">`;}};el.querySelector(".mr-save").onclick=saveForm;
  }
  ...
  async function saveForm(){
    ...
    if(!name)return showToast("Reward name cannot be empty.");if(!Number.isInteger(cost)||cost<0)return showToast("Points must be a whole number.");if(!category)return showToast("Please enter a category.");
    let stock=null;if(stockRaw!==""){stock=Number(stockRaw);if(!Number.isInteger(stock)||stock<0)return showToast("Stock must be 0 or a whole number.");}
    const button=document.querySelector(".mr-save");if(button){button.disabled=true;button.textContent="Saving...";}
    try{
      let image=editingId==null?null:((window.rewards||[]).find(r=>Number(r.id)===editingId)?.image||null);
      if(pendingFile){const ext=pendingFile.name.includes(".")?pendingFile.name.split(".").pop().toLowerCase():"jpg",path=`reward-${editingId||"new"}-${Date.now()}.${ext}`,up=await window.supabaseClient.storage.from(IMAGE_BUCKET).upload(path,pendingFile,{upsert:false,contentType:pendingFile.type});if(up.error)throw up.error;image=path;}
      if(editingId!=null){const old=(window.rewards||[]).find(r=>Number(r.id)===editingId),res=await window.supabaseClient.from("rewards").update({name,cost,category,stock,image}).eq("id",editingId);if(res.error)throw res.error;if(old?.image&&image!==old.image)await window.supabaseClient.storage.from(IMAGE_BUCKET).remove([old.image]);}
      ...
      const wasEditing=editingId!=null;await openManager();showToast(wasEditing?"Reward updated successfully":"Reward added successfully");
  ...
  window.manageRewardEdit=id=>openForm(id);window.manageRewardMove=move;window.manageRewardToggle=toggle;
