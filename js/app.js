...
function getAvatar(name) {
  // Avatar identity is intentionally tied to the family member's current name/alias,
  // not to the old English names, so renaming an account does not make its avatar disappear.
  const value = String(name || "").trim().toLowerCase().replace(/\s+/g, "");
  if (value === "饲养员" || value.includes("饲养员")) return "🌙";
  if (value === "老朱" || value === "oldzhu" || value === "老猪" || value.includes("老朱")) return "🐷";
  if (value === "狗屎" || value === "shit" || value.includes("狗屎")) return "💩";
  if (value === "大厨" || value === "chef" || value.includes("大厨")) return chefAvatarSvg();
  return "👤";
}
...
function renderAccountMenu() {
  ...
  list.innerHTML = profiles.map(profile => {
    ...
    return `<button class="account-option ${active ? "active" : ""}" onclick="switchProfile(${profile.id})"><span class="account-option-avatar">${avatar}</span><span class="account-option-info"><strong>${escapeHtml(getProfileName(profile))}</strong><small>${role} · ${Number(profile.balance || 0).toLocaleString()} pts</small></span>${active ? "<span>✓</span>" : ""}</button>`;
  }).join("");
}
...
async function loadCloudData() {
  ...
  const profileResult = await supabaseClient.from("profiles").select("id,name,balance,role").eq("id", currentProfile.id).single();
  ...
  const historyResult = await supabaseClient.from("redemptions").select("id,user_id,reward_id,reward_name,cost,created_at").eq("user_id", currentProfile.id).order("created_at", { ascending: false });
...
function renderProducts() {
  ...
  grid.innerHTML = visibleRewards.map(reward => {
    ...
    const editButton = isParent() ? `<button class="edit-name-button" onclick="openEditReward(${reward.id})">⚙️ Edit Reward</button>` : "";
    const imageHtml = image ? `<img src="${escapeHtml(image)}" alt="${name}">` : `<div class="product-icon">${icon}</div>`;
    return `<article class="card"><div class="product-image">${imageHtml}</div><div class="product-content"><div class="product-name">${name}</div><div class="product-cost">${cost.toLocaleString()} points</div><button class="redeem-button" ${canRedeem ? "" : "disabled"} onclick="openRedeem(${reward.id})">${canRedeem ? "Redeem" : "Not enough points"}</button>${editButton}</div></article>`;
  }).join("");
}
