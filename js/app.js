...
function getAvatar(name) {
  // Keep these aliases so changing an account name does not remove its avatar.
  const value = String(name || "").trim().toLowerCase().replace(/\s+/g, "");
  if (value === "饲养员" || value.includes("饲养员")) return "🌙";
  if (value === "老朱" || value === "老猪" || value === "oldzhu" || value === "old猪" || value.includes("老朱")) return "🐷";
...
async function confirmRedeem() {
  ...
  try {
    ...
    if (historyInsert.error) {
      await supabaseClient.from("profiles").update({ balance }).eq("id", currentProfile.id);
